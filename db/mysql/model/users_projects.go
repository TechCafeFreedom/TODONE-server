...
var UsersProjectWhere = struct {
	...
	ID:        whereHelperint{field: "`users_projects`.`id`"},
	UserID:    whereHelperstring{field: "`users_projects`.`user_id`"},
	ProjectID: whereHelperint{field: "`users_projects`.`project_id`"},
	Role:      whereHelperint8{field: "`users_projects`.`role`"},
}
...
var (
	usersProjectAllColumns            = []string{"id", "user_id", "project_id", "role"}
	usersProjectColumnsWithoutDefault = []string{"user_id", "project_id", "role"}
	usersProjectColumnsWithDefault    = []string{"id"}
	usersProjectPrimaryKeyColumns     = []string{"id"}
)

type (
	// UsersProjectSlice is an alias for a slice of pointers to UsersProject.
	// This should generally be used opposed to []UsersProject.
...
func (q usersProjectQuery) One(ctx context.Context, exec boil.ContextExecutor) (*UsersProject, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "model: failed to execute a one query for users_projects")
	}
...
func (q usersProjectQuery) All(ctx context.Context, exec boil.ContextExecutor) (UsersProjectSlice, error) {
	...
	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "model: failed to assign all query results to UsersProject slice")
	}
...
func (q usersProjectQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to count users_projects rows")
	}
...
func (q usersProjectQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "model: failed to check if users_projects exists")
	}
...
func (usersProjectL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUsersProject interface{}, mods queries.Applicator) error {
	...
	if singular {
		object = maybeUsersProject.(*UsersProject)
	} else {
		slice = *maybeUsersProject.(*[]*UsersProject)
	}
...
func UsersProjects(mods ...qm.QueryMod) usersProjectQuery {
	...
	return usersProjectQuery{NewQuery(mods...)}
}

// FindUsersProject retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindUsersProject(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*UsersProject, error) {
	...
	query := fmt.Sprintf(
		"select %s from `users_projects` where `id`=?", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "model: unable to select from users_projects")
	}
...
func (o *UsersProject) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("model: no users_projects provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `users_projects` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `users_projects` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, usersProjectPrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to insert into users_projects")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == usersProjectMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to populate default values for users_projects")
	}
...
func (o *UsersProject) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("model: unable to update users_projects, could not build whitelist")
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to update users_projects row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by update for users_projects")
	}
...
func (q usersProjectQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to update all for users_projects")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to retrieve rows affected for users_projects")
	}
...
func (o UsersProjectSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `users_projects` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, usersProjectPrimaryKeyColumns, len(o)))
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to update all in usersProject slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to retrieve rows affected all in update all usersProject")
	}
	return rowsAff, nil
}

var mySQLUsersProjectUniqueColumns = []string{
	"id",
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *UsersProject) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("model: no users_projects provided for upsert")
	}
	...
	nzDefaults := queries.NonZeroDefaultSet(usersProjectColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLUsersProjectUniqueColumns, o)
	...
	if !cached {
		...
		if len(update) == 0 {
			return errors.New("model: unable to upsert users_projects, could not build update column list")
		}

		ret = strmangle.SetComplement(ret, nzUniques)
		cache.query = buildUpsertQueryMySQL(dialect, "users_projects", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `users_projects` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to upsert for users_projects")
	}
	...
	uniqueMap, err = queries.BindMapping(usersProjectType, usersProjectMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "model: unable to retrieve unique values for users_projects")
	}
...
func (o *UsersProject) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("model: no UsersProject provided for delete")
	}
	...
	sql := "DELETE FROM `users_projects` WHERE `id`=?"
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to delete from users_projects")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by delete for users_projects")
	}
...
func (q usersProjectQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if q.Query == nil {
		return 0, errors.New("model: no usersProjectQuery provided for delete all")
	}
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to delete all from users_projects")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by deleteall for users_projects")
	}
...
func (o UsersProjectSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to delete all from usersProject slice")
	}
...
func (o *UsersProject) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindUsersProject(ctx, exec, o.ID)
	if err != nil {
		return err
	}
...
func (o *UsersProjectSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to reload all in UsersProjectSlice")
	}
...
func UsersProjectExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `users_projects` where `id`=? limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "model: unable to check if users_projects exists")
	}
